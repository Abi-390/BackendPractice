...
})(UserRoles || (UserRoles = {})); // enum is kind of similar to objects but we use it to create constant values ( you can't create instances of an enum) where we can in future access admin like UserRoles.ADMIN or guest like UserRoles.GUEST etc
...
var foodMaker = /** @class */ (function () {
    function foodMaker(key) {
        this.key = key;
...
var f1 = new foodMaker("abc"); // here we are creating an object of class foodMaker and specifying the type parameter T as string, so the key property of this object will be of type string and we are passing a string value "abc" for key property.
var f2 = new foodMaker(123); // here we are creating an object of class foodMaker and specifying the type parameter T as number, so the key property of this object will be of type number and we are passing a number value 123 for key property.
// --- Exception note for genrics in ts ---
function Exception(a, b) {
    return 'hey'; // here we have defined a function Exception that takes two parameters a and b of type T and returns a value of type T, but in the function body we are returning a string value "hey" and using type assertion to assert that it is of type T, this is an exception in TypeScript where we can use type assertions to override the type checking and tell the compiler to treat a value as a different type, but we should use this feature with caution because it can lead to runtime errors if we assert a value to be of a type that it is not actually compatible with.
    // or return <T>'hey';
}
Exception('hello', 'world'); // here we are calling Exception function and specifying the type parameter T as string, so it will return a string value 'hey', but since we are using a type assertion to assert that 'hey' is of type T (which is string in this case), it will not give an error even though 'hey' is not actually of type string, this is an exception in TypeScript where we can use type assertions to override the type checking and tell the compiler to treat a value as a different type, but we should use this feature with caution because it can lead to runtime errors if we assert a value to be of a type that it is not actually compatible with.
